...
- Server 通过本地 JSON store 持久化 tunnel，管理 API 可动态创建、删除、重启 tunnel。
- 每个 tunnel 使用独立 token 鉴权，未鉴权 agent 不能参与流量转发。
- 管理接口使用 HTTP Basic Auth 保护。
- 无已鉴权 agent 时，用户访问开放端口应快速失败。
- 提供 tunnel 运行状态和生命周期查询，便于自用排障。
...
- 管理端通过 `/api/tunnels` 创建 tunnel、查看 agent 是否在线和当前连接数。

## 验收标准

- 正确 token 下 TCP/HTTP 透传保持可用。
- 错误 token 或未认证 agent 不能接管 tunnel。
- 管理 API 无 Basic Auth 或用户名密码错误时返回 401，并触发浏览器登录弹窗。
- API 和日志不输出 tunnel token 或管理端密码。
- 创建/reset token 时明文 agent token 只返回一次，store 只保存 PBKDF2 hash。
